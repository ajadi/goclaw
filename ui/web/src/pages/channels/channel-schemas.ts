...
export const configSchema: Record<string, FieldDef[]> = {
  ...
  pancake: [
    ...
    { key: "features.private_reply", label: "Private Reply (Comment → DM)", type: "boolean", defaultValue: false,
      help: "Send a private DM to commenters. Facebook/Instagram only. DM must be sent within 7 days of the original comment (Meta policy).",
      showWhen: { key: "platform", value: ["facebook", "instagram"] } },
    { key: "private_reply_mode", label: "DM Mode", type: "select", defaultValue: "after_reply",
      options: [
        { value: "after_reply", label: "After public reply (default)" },
        { value: "standalone", label: "DM only (no public reply)" },
      ],
      help: "Standalone bypasses the comment keyword filter — DM runs on every valid commenter.",
      showWhen: { key: "features.private_reply", value: "true" } },
    { key: "private_reply_message", label: "DM Message", type: "textarea",
      help: "Supports {{commenter_name}} and {{post_title}}. Empty = default text.",
      placeholder: "Hi {{commenter_name}}! Thanks for commenting on \"{{post_title}}\". How can I help?",
      showWhen: { key: "features.private_reply", value: "true" } },
    { key: "private_reply_ttl_days", label: "Re-DM After (days)", type: "number", defaultValue: 7,
      help: "Days before the same commenter can receive the auto-DM again. Default 7.",
      showWhen: { key: "features.private_reply", value: "true" } },
    { key: "private_reply_options.allow_post_ids", label: "Allow Post IDs", type: "tags",
      help: "Only DM on these post IDs. Empty = all posts.",
      showWhen: { key: "features.private_reply", value: "true" } },
    { key: "private_reply_options.deny_post_ids", label: "Deny Post IDs", type: "tags",
      help: "Skip DM on these post IDs.",
      showWhen: { key: "features.private_reply", value: "true" } },
